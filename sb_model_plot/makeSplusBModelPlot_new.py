...
def getLimits(results_path, do_NMSSM = False):
  ...
  if do_NMSSM:
    m = {}
    for line in results:
      mx = int( line.split("my")[0].split("mx")[-1] )
      my = int( line.split("mh")[0].split("my")[-1] )
      if mx not in m: m[mx] = [my]
      else:
        if my not in m[mx]: m[mx].append(my)

    limits, limits_no_sys, limits_no_res_bkg = {}, {}, {}
    for mx in m: limits[mx] = np.zeros((5, len(m[mx])))
    for mx in m: limits_no_sys[mx] = np.zeros((5, len(m[mx])))
    for mx in m: limits_no_res_bkg[mx] = np.zeros((5, len(m[mx])))

    for line in results:
      mx = int( line.split("my")[0].split("mx")[-1] )
      my = int( line.split("mh")[0].split("my")[-1] )
      idx1 = np.where( np.array(m[mx]) == my )[0][0]
      if "2.5%" in line:
        idx2=0
      elif "16.0%" in line:
        idx2=1
      ...
      if "no_sys" in line:
        limits_no_sys[mx][idx2][idx1] = limit
      elif "no_res_bkg" in line:
        limits_no_res_bkg[mx][idx2][idx1] = limit
      else:
        limits[mx][idx2][idx1] = limit

    return m, limits, limits_no_sys, limits_no_res_bkg

  else:
    mx = []
    for line in results:
      #m = int(line.split(".")[0].split("_")[-1])
      m = int( line.split("my")[0].split("mx")[-1] )
      mx.append(m)
    mx = np.array(sorted(set(mx)))

    limits = np.zeros((5, len(mx)))
    limits_no_sys = np.zeros((5, len(mx)))
    limits_no_res_bkg = np.zeros((5, len(mx)))
    ...
    return mx, limits, limits_no_sys, limits_no_res_bkg
...
def get_options():
  ...
  parser.add_option("--inputResultsFile", dest="inputResultsFile", default=None, help="Input results file")
...
mx, limits, limits_no_sys, limits_no_res_bkg = getLimits( opt.inputResultsFile, opt.do_NMSSM )
if opt.do_NMSSM:
  limit = limits[float(opt.MX)][2][np.where(np.array(mx[float(opt.MX)]) == float(opt.MY))[0][0]]
else:
  limit = limits[2][np.where(mx==float(opt.MX))[0][0]]
...
# Set resonance mass
if opt.MX is not None: w.var("MX").setVal(float(opt.MX))
if opt.MY is not None: w.var("MY").setVal(float(opt.MY))
...
for cidx in range(len(cats)):
  ...
  # If option doZeroes: add error of +-1.8325 for zero bins
  if opt.doZeroes:
    for ibin in range(1,h_data.GetNbinsX()+1):
      bcenter = h_data.GetBinCenter(ibin)
      # Skip blinded region
  ...
  # Scale signal histogram by expected limit
  h_spdf['pdfNBins'].Scale(limit)
  h_spdf['nBins'].Scale(limit)
  ...
  #obs_limit = 2.71
  #_factor = obs_limit/w.var("r").getVal()
 
  if not opt.skipIndividualCatPlots:
    print "    * making plot"
    if not os.path.isdir("./SplusBModels%s"%(opt.ext)): os.system("mkdir ./SplusBModels%s"%(opt.ext))
    if opt.doBands: makeSplusBPlot(w,h_data,h_sbpdf,h_bpdf,h_spdf,h_data_ratio,h_bpdf_ratio,h_spdf_ratio,c,opt,df_bands,_reduceRange, limit=limit)
    else: makeSplusBPlot(w,h_data,h_sbpdf,h_bpdf,h_spdf,h_data_ratio,h_bpdf_ratio,h_spdf_ratio,c,opt,None,_reduceRange, limit=limit)
...
if( len(opt.cats.split(",")) > 1 )|( opt.cats == 'all' ):
  ...
  if opt.doSumCategories:
    ...
    if opt.doBands: makeSplusBPlot(w,h_data_sum,h_sbpdf_sum,h_bpdf_sum,h_spdf_sum,h_data_ratio_sum,h_bpdf_ratio_sum,h_spdf_ratio_sum,'all',opt, df_bands,_reduceRange, limit=limit)
    else: makeSplusBPlot(w,h_data_sum,h_sbpdf_sum,h_bpdf_sum,h_spdf_sum,h_data_ratio_sum,h_bpdf_ratio_sum,h_spdf_ratio_sum,'all',opt,None,_reduceRange,factor, limit=limit)
    if opt.doCatWeights:
      print " --> Making weighted plot for sum of categories"
      if opt.doBands: makeSplusBPlot(w,h_wdata_sum,h_wsbpdf_sum,h_wbpdf_sum,h_wspdf_sum,h_wdata_ratio_sum,h_wbpdf_ratio_sum,h_wspdf_ratio_sum,'wall',opt, df_bands, _reduceRange, limit=limit)
      else: makeSplusBPlot(w,h_wdata_sum,h_wsbpdf_sum,h_wbpdf_sum,h_wspdf_sum,h_wdata_ratio_sum,h_wbpdf_ratio_sum,h_wspdf_ratio_sum,'wall',opt, None, _reduceRange, limit=limit)
